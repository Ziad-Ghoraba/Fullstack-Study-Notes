...
***
- **The Dynamic Routing** refers to routes that are created based on dynamic values or data passed at runtime and you can define it like this ...  
  `path: 'users/:userId'` (By adding a colon and then an identifier that can be any name but you will use the same name later).
...
    children: [
      {
        path: 'tasks', // Path will be concatenated.
        component: TasksComponent,
      },
      {
        path: 'tasks/new',
...
        // To make these values inherited.
      })
    ),
  ],
}; 
...
***
- If you need to perform programmatic navigation (Like when the user click on some button) do some navigation you would first inject `Router` service and use it to navigate inside the `onSubmit` method for example.
...
  routerState: RouterStateSnapshot // Always pass those two values.
) => {
...
- Note that it takes the component it will be worked with.
