...
****
 Note that angular automatically detect when a component's data has changed .. and it checks if this change will reflect on the UI .. if it will then it just updates the UI automatically .. and this is done using **zone.js** library.
***
...
****
 `@Input() name?: string` this question mark tells TS that this input might be unset and I'm aware of that .. or you could use something like this : `string | undefined`.
...
```
 ...
 This is used to output the list users dynamically and the keyword `track` is used to make each item has a specific thing that's attached to it .. this helps angular a lot when it needs to be rendered again and make it a way more efficient.

 Note `@if & @else if & @else` use.
****
 Note the use of the legacy code with `ngFor & ngif` directives.
***
 It's a common pattern that you use a separate files for the interfaces (or types) .. and then import and add the prefix keyword `type` to make it very clear that you are importing a type here .. and with this you now can use this interface anywhere in your application.
...
****
 You could use **pipes** to do some changes to the output before outputting it .. they are used by this `|` which's put after the output like the following ...
