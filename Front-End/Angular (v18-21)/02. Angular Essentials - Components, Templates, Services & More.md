...
****
Angular CLI compiles TypeScript into JavaScript bundles and automatically injects those bundles into index.html so the browser can execute the app.

Think like this:
> You write TypeScript → Angular converts it to JavaScript→ Browser runs JavaScript

****
 We are building components as classes but we never make objects of them; angular will do this.
 Angular **instantiates components automatically** when:
- It finds `<app-root>` in HTML "Keep in mind app-root just an example :)"
- It builds the component tree
- It runs dependency injection
